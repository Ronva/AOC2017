input = String.split("ba dec 37 if znx != 0
...
testInput = String.split("b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10", "\n")
...
defmodule Day8 do
  def separate(list) do
    split = Enum.map(list, &(String.split(&1)))
    registers = Enum.map(split, &(Enum.at(&1, 0)))
    |> Enum.uniq()
    |> Enum.map(fn(x) -> {x, 0} end)
    |> Map.new
    operations(split, registers)
  end

  def operations(ops, map, i\\ 0, max\\ 0) do
    if i < length(ops) do
      op = Enum.at(ops, i)
      toUpdate = Enum.at(op, 0)
      dir = Enum.at(op, 1)
      amount = indToInt(op, 2)
      var1 = map[Enum.at(op, 4)]
      cond = Enum.at(op, 5)
      var2 = indToInt(op, 6)
      eval = evaluate(var1, cond, var2)
      newMap = if eval do
        Map.update(map, toUpdate, map[toUpdate], &(&1 + if dir == "inc", do: amount, else: amount * -1))
      else
        map
      end
      curMax = Map.values(map) |> Enum.max()
      newMax = if eval && curMax > max, do: curMax, else: max
      operations(ops, newMap, i + 1, newMax)
    else
      IO.inspect("max: #{max}")
      Map.values(map)
      |> Enum.max()
  ...
  def indToInt(list, index) do
    Enum.at(list, index)
    |> String.to_integer()
  end

  def evaluate(var1, cond, var2) do
    case cond do
      ">" -> var1 > var2
      "<" -> var1 < var2
      ">=" -> var1 >= var2
  ...
  def incDec(val1, val2, op) do
    if op == "inc", do: val1 + val2, else: val1 - val2
  end
end

IO.inspect(Day8.separate(input))